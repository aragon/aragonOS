const { assertBn, bn, getEventArgument } = require('@aragon/contract-helpers-test')
  ...
  for (const { title, tokenContract } of testGroups) {
    context(`> ${title}`, () => {
      ...
      it('can correctly transfer', async () => {
        ...
        assertBn((await tokenMock.balanceOf(owner)), initialBalance, 'Balance of owner should be correct')
        assertBn((await tokenMock.balanceOf(safeERC20Mock.address)), 0, 'Balance of mock should be correct')
      })
      ...
      it('detects failed transfer', async () => {
        ...
        assertBn(await tokenMock.balanceOf(owner), initialBalance, 'Balance of owner should stay the same')
      ...
      it('can correctly approve', async () => {
        const approvedAmount = bn(5000)

        // Create approval from the mock
        const receipt = await safeERC20Mock.approve(tokenMock.address, receiver, approvedAmount)
        assertMockResult(receipt, true)
        assertBn(await tokenMock.allowance(safeERC20Mock.address, receiver), approvedAmount, 'Allowance of receiver should be correct')
      })

      it('detects failed approve', async () => {
        const preApprovedAmount = bn(5000)
      ...
      it('detects failed transferFrom', async () => {
        ...
        assertMockResult(receipt, false)
        assertBn(await tokenMock.balanceOf(owner), initialBalance, 'Balance of owner should stay the same')
        assertBn(await tokenMock.balanceOf(receiver), 0, 'Balance of receiver should stay the same')
        assertBn(await tokenMock.balanceOf(safeERC20Mock.address), 0, 'Balance of mock should stay the same')
      })
      ...
      it('gives correct value with static allowance', async () => {
        ...
        const approval = (await safeERC20Mock.allowance(tokenMock.address, owner, safeERC20Mock.address))
        assertBn(approval, approvedAmount, 'Mock should return correct allowance')
        assertBn(await tokenMock.allowance(owner, safeERC20Mock.address), approval, "Mock should match token contract's allowance")
      })

      it('gives correct value with static balanceOf', async () => {
        const balance = await safeERC20Mock.balanceOf(tokenMock.address, owner)
        assertBn(balance, initialBalance, 'Mock should return correct balance')
        assertBn(await tokenMock.balanceOf(owner), balance, "Mock should match token contract's balance")
      })

      it('gives correct value with static totalSupply', async () => {
        const totalSupply = (await safeERC20Mock.totalSupply(tokenMock.address))
        assertBn(totalSupply, initialBalance, 'Mock should return correct total supply')
        assertBn(await tokenMock.totalSupply(), totalSupply, "Mock should match token contract's total supply")
      })
    })
  }
})
