...
const SampleApp = artifacts.require('RelayerAragonAppWithParameterizedSenderMock')
...
contract('ParameterizedRelayerApp', ([_, root, sender, vault, offChainRelayerService]) => {
  ...
  beforeEach('relay transaction', async () => {
    const calldata = app.contract.relayedWrite.getData(sender, 10)
  ...
  it('overloads a transaction with ~75k of gas', async () => {
    ...
    assert.isBelow(gasOverload, 75000, 'relayed txs gas overload is higher than 75k')
  })
})
