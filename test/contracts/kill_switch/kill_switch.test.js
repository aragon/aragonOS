...
contract('KillSwitch', ([_, root, owner, securityPartner, anyone]) => {
  ...
  let CORE_NAMESPACE, KERNEL_APP_ID, APP_MANAGER_ROLE, SET_SEVERITY_ROLE, SET_DEFAULT_ISSUES_REGISTRY_ROLE, SET_ISSUES_REGISTRY_ROLE, SET_CONTRACT_ACTION_ROLE, SET_HIGHEST_ALLOWED_SEVERITY_ROLE
  ...
  before('load constants and roles', async () => {
    ...
    SET_CONTRACT_ACTION_ROLE = await killSwitchBase.SET_CONTRACT_ACTION_ROLE()
  ...
  context('when the kernel version does support kill-switch logic', async () => {
    ...
    context('when the kernel was initialized with a kill-switch', async () => {
      ...
      describe('gas costs', () => {
        ...
        it('kill switch should overload ~16k of gas to a function', skipCoverage(async () => {
          ...
          assert(killSwitchCost <= 16000, 'kill switch should overload ~16k of gas')
        }))
      })
    })
  })
