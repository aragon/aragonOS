...
contract CompanyConfiguratorFactory {
  ...
  function configureCompany(address companyAddress, address god) only(companyDeployer[companyAddress]) {
    ICompany company = ICompany(companyAddress);
  ...
  function createVotingStock(ICompany company, address god) private {
    VotingStock stock = new VotingStock(address(company));
    company.addStock(address(stock), 1);
    company.grantStock(0, 1, god);
  }

  function setExecutives(ICompany company, address executive) {
    company.setEntityStatusByStatus(executive, 2);
  }

  function setInitialBylaws(ICompany company) {
    ...
    company.setStatusBylaw("beginPoll(address,uint64,bool,bool)", uint8(ICompany.SpecialEntityStatus.Shareholder), true);
    ...
    company.setStatusBylaw("grantStock(uint8,uint256,address)", uint8(ICompany.EntityStatus.Executive), false);
    ...
    company.setStatusBylaw("transferSaleFunds(uint256)", uint8(ICompany.EntityStatus.Executive), false);

    company.setVotingBylaw("setAccountingSettings(uint256,uint64,uint256)", 1, 2, true, minimumVotingTime, favor);
    company.setStatusBylaw("createRecurringReward(address,uint256,uint64,string)", uint8(ICompany.EntityStatus.Executive), false);

    company.setStatusBylaw("removeRecurringReward(uint)", uint8(ICompany.EntityStatus.Executive), false);
    company.setStatusBylaw("issueReward(address,uint256,string)", uint8(ICompany.EntityStatus.Executive), false);

    company.setStatusBylaw("assignStock(uint8,address,uint256)", uint8(ICompany.SpecialEntityStatus.StockSale), true);
    company.setStatusBylaw("removeStock(uint8,address,uint256)", uint8(ICompany.SpecialEntityStatus.StockSale), true);
