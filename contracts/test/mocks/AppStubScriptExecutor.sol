...
contract AppStubScriptExecutor is AragonApp {
    // Initialization is required to access any of the real executors
    function initialize() public {
        initialized();
    }

    function execute(bytes script) public returns (bytes) {
        return runScript(script, new bytes(0), new address[](0));
    }

    function executeWithBan(bytes script, address[] memory blacklist) public returns (bytes) {
