...
contract AccountingApp {
    ...
    tid = Transation(value:3, 
     ...
     aid = TransactionAction(transactionId: tid, 
                             state: New, 
                             reason:"new invoice", 
                             actor: ContractBob)
     ...
     aid = TransactionAction(transactionId: tid, 
                             state: Approved, 
                             reason:"verified the invoice", 
                             actor: AdminSam)
     tid.transactionStates.push(aid)
    
     3) Alice attemtps to with draw and now it is in the state where someone has a last chance to cancel it.
     aid = TransactionAction(transactionId: tid, 
                             state: Pending, 
                             reason:"withdawing", 
                             actor: Alice)
     ...
     5.a) Yay, Eves address accepts this token!
     aid = TransactionAction(transactionId: tid, 
                             state: Succeeded, 
                             reason:"", 
                             actor: VaultOrgan)
     ...
     aid = TransactionAction(transactionId: tid, 
                             state: Canceled, 
                             reason:"This is a fake invoice", 
                             actor: SomeOtherAdmin)
     tid.transactionStates.push(aid)
    */
