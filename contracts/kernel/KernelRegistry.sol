...
contract KernelRegistry is IKernelRegistry, DAOStorage {
    ...
    function register(address impl, bytes4[] sigs, bool delegate) internal {
        ...
        bytes4 id = bytes4(sha3(sigs));
        storageSet(storageKeyForSig(id), identifier(delegate));
    ...
    function deregister(bytes4[] sigs, bool delegate) internal {
        ...
        require(storageGet(storageKeyForSig(id)) == identifier(delegate));
        for (uint i = 0; i < sigs.length; i++) {
            storageSet(storageKeyForSig(sigs[i]), 0);
        }

        Deregister(id, delegate);
    }
