...
contract OwnershipConstants {
    bytes4 constant ADD_TOKEN_SIG = bytes4(sha3("addToken(address,uint256,uint128,uint128)"));
    bytes4 constant REMOVE_TOKEN_SIG = bytes4(sha3("removeToken(address)"));
    bytes4 constant GET_TOKEN_SIG = bytes4(sha3("getToken(uint256)"));
    bytes4 constant ISSUE_TOKENS_SIG = bytes4(sha3("issueTokens(address,uint256)"));
    bytes4 constant GRANT_TOKENS_SIG = bytes4(sha3("grantTokens(address,address,uint256)"));
    bytes4 constant GRANT_VESTED_TOKEN_SIG = bytes4(sha3("grantVestedTokens(address,address,uint256,uint64,uint64,uint64)"));
}
...
contract OwnershipApp is OwnershipConstants, Application, Controller, Requestor {
    ...
    function canHandlePayload(bytes payload) constant returns (bool) {
        ...
        return(
            sig == ADD_TOKEN_SIG ||
            sig == REMOVE_TOKEN_SIG ||
            sig == ISSUE_TOKENS_SIG ||
            sig == GRANT_TOKENS_SIG ||
            sig == GRANT_VESTED_TOKEN_SIG ||
            sig == GET_TOKEN_SIG ||
            sig == CREATE_TOKEN_SALE_SIG ||
            sig == CLOSE_TOKEN_SALE_SIG ||
