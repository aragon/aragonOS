...
contract VariablePriceSale is TokenSale {
  ...
  function instantiate(address _dao, OwnershipApp _ownershipApp, ERC20 _raiseToken, ERC20 _saleToken, uint _cap, uint _minBuy, bool _isInverseRate, uint64 _startBlock, uint64[] _periodEnds, uint[] _prices) {
    ...
    for (uint i = 0; i < _periodEnds.length; i++) {
      ...
      if (i > 0) require(periods[i - 1].periodEnds < periods[i].periodEnds);
    }
  }
